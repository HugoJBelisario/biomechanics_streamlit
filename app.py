...
def prepare_biodex_fatigue_trend_data(peak_torque_rows, auc_rows, series_col="Athlete"):
    """Build the (long_df, metric_cols) pair build_biodex_fatigue_trend_figure needs.
    ...
    as a set goes on, the warp stretches that away instead of showing it. This instead
    exposes the underlying per-rep scalars (already computed for the Peak Torque/AUC
    tables) directly against rep number, so a fatigue-driven trend shows up as a slope
    across reps rather than being normalized out of the picture.
    ...
    Phase View leaves in — only columns actually present are offered, so this adapts to
    Full/Internal/External automatically. `series_col` names the column that
    distinguishes one line from another (e.g. "Athlete" for Session mode's per-session
    rows, or a "Group" column for Custom Groups' per-group rows) — pass a list of
    column names to combine several into one composite series label.

    Returns (None, []) if there's nothing to plot (fewer than 2 reps for every series,
    or no recognized metric columns).
    """
    combined_rows = []
    peak_by_key = {}
    for row in peak_torque_rows or []:
        key = (row.get("Athlete"), row.get("Session Date"), row.get("Group"), row.get("Rep"))
        peak_by_key[key] = row
    ...
    series_cols = series_col if isinstance(series_col, list) else [series_col]
    series_cols = [col for col in series_cols if col in df.columns]
    if not series_cols:
        return None, []
...
def build_biodex_fatigue_trend_figure(df, metric_col, series_color_map=None):
    """Single line chart of `metric_col` vs. rep number, one line per `df["__series__"]`.

    Rep-number tick labels are rotated vertical (not the usual horizontal) so every rep
    stays readable instead of Plotly thinning them out to avoid overlap on a wide range.
    """
    fig = go.Figure()
    for series_name in sorted(df["__series__"].unique()):
    ...
    fig.update_xaxes(title_text="Rep", dtick=1, tickangle=90)
    fig.update_layout(
...
PELVIS_TORSO_COMPONENT_METRIC_MAP = {
    ...
    display_label: ("RPV_DIST", f"JCS_{category_kind}_{category_component}")
    for category_component, display_labels in PELVIS_COMPONENT_LABELS_BY_AXIS.items()
    for display_label, category_kind in zip(
        display_labels,
        ("TORQUE", "ANGVEL", "ANGACCEL"),
    )
}
...
ARM_ENERGY_FLOW_COMPONENT_CONFIG = {
    ...
    "Arm Horizontal Abd/Add Energy Flow": ("Horizontal Abduction/Adduction", "HORIZABD"),
}

ARM_ENERGY_COMPONENT_METRIC_MAP = {
    f"Arm {display_component} {display_kind}": f"JCS_{category_kind}_{category_component}"
    for display_component, category_component in ARM_ENERGY_FLOW_COMPONENT_CONFIG.values()
    for display_kind, category_kind in (
        ("Torque", "TORQUE"),
        ("Angular Velocity", "ANGVEL"),
        ("Angular Acceleration", "ANGACCEL"),
...
ENERGY_FLOW_COMPONENT_OPTIONS_BY_PRIMARY = {
    ...
    **{
        primary_metric: (
            list(PELVIS_COMPONENT_LABELS_BY_AXIS[_category_component])
            if body_region == "Pelvis" else
            [
...
ARM_ENERGY_COMPONENT_COLOR_MAP = {
    "Arm Elevation/Depression Torque": "#F97316",
    "Arm Elevation/Depression Angular Velocity": "#2563EB",
    "Arm Elevation/Depression Angular Acceleration": "#9333EA",
    "Arm Rotational Torque": "#DC2626",
    "Arm Rotational Angular Velocity": "#0891B2",
    "Arm Rotational Angular Acceleration": "#7C3AED",
    "Arm Horizontal Abduction/Adduction Torque": "#16A34A",
    "Arm Horizontal Abduction/Adduction Angular Velocity": "#D97706",
...
ENERGY_METRIC_RENAMES = {
    ...
    "Torso Flexion/Extension Energy Flow": "Torso-Pelvis Flexion/Extension Energy Flow",
    "Torso Side Bending Energy Flow": "Torso-Pelvis Lateral Flexion Energy Flow",
    "Torso Flexion/Extension Torque": "Torso-Pelvis Flexion/Extension Torque",
    "Torso Flexion/Extension Angular Velocity": "Torso-Pelvis Flexion/Extension Angular Velocity",
    "Torso Flexion/Extension Angular Acceleration": "Torso-Pelvis Flexion/Extension Angular Acceleration",
...
PELVIS_TORSO_TORQUE_METRICS = [
    ...
    metric for metric in PELVIS_TORSO_COMPONENT_METRIC_MAP
    if metric.startswith("Torso-Pelvis ") and metric.endswith(" Torque")
] + [
    metric
    for metric, (_segment_name, category_name) in PELVIS_TORSO_COMPONENT_METRIC_MAP.items()
    if category_name.startswith("JCS_TORQUE_") and metric.startswith("Pelvis ")
]
...
ENERGY_TORQUE_METRICS = {
    ...
    *{
        metric
        for metric, (_segment_name, category_name) in PELVIS_TORSO_COMPONENT_METRIC_MAP.items()
        if category_name.startswith("JCS_TORQUE_")
    },
...
ENERGY_ANGULAR_VELOCITY_METRICS = {
    ...
    *{
        metric
        for metric, (_segment_name, category_name) in PELVIS_TORSO_COMPONENT_METRIC_MAP.items()
        if category_name.startswith("JCS_ANGVEL_")
    },
...
ENERGY_ANGULAR_ACCELERATION_METRICS = {
    ...
    *{
        metric
        for metric, (_segment_name, category_name) in PELVIS_TORSO_COMPONENT_METRIC_MAP.items()
        if category_name.startswith("JCS_ANGACCEL_")
    },
    ...
    if energy_view_mode == "Comparison":
        ...
        with energy_left_select_col:
            left_energy_metrics = st.multiselect(
                "Select Primary Energy Flow",
                energy_metric_options,
                default=[],
                key="energy_comparison_left_metrics",
            )
        right_energy_metric_options = list(dict.fromkeys(
            related_metric
        ...
        if right_component_key in st.session_state:
            st.session_state[right_component_key] = list(dict.fromkeys(
                ENERGY_METRIC_RENAMES.get(metric, metric)
                for metric in st.session_state[right_component_key]
                if ENERGY_METRIC_RENAMES.get(metric, metric) in right_energy_metric_options
            ))
                ...
                if not selected_sessions:
                    ...
                    if compare_display_mode == "Individual Reps":
                        ...
                        fatigue_df, fatigue_metric_cols = prepare_biodex_fatigue_trend_data(
                            session_peak_torque_rows, session_auc_rows, series_col=["Athlete", "Session Date"],
                        )
